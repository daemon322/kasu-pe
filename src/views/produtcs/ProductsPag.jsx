    ...
    <div className="space-y-2">
      ...
      <div className="relative h-1.5 bg-white/10 rounded-full mx-1">
        <div
          ...
          style={{ left: `${pct0}%`, right: `${100 - pct1}%` }}
        />
        <input
          type="range"
          min={min}
          ...
          value={value[0]}
          onChange={(e) =>
            +e.target.value < value[1] && onChange([+e.target.value, value[1]])
          }
        ...
        <input
          ...
          value={value[1]}
          onChange={(e) =>
            +e.target.value > value[0] && onChange([value[0], +e.target.value])
          }
          className="absolute inset-0 w-full opacity-0 cursor-pointer h-full"
        />
      </div>
    ...
    <div className="space-y-5" style={{ fontFamily: "'Nunito',sans-serif" }}>
      ...
      <div>
        ...
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 focus-within:border-amber-500 rounded-xl px-3 py-2.5 transition-all">
          ...
          {query && (
            <button
              onClick={() => setQuery("")}
              className="text-slate-600 hover:text-white text-lg leading-none"
            >
              ×
            </button>
          )}
      ...
      <div>
        ...
        <div className="space-y-0.5">
          ...
          >
            <span>🗂️ Todos</span>
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeCategory === "todos" ? "bg-black/20" : "bg-white/8 text-slate-500"}`}
            >
              {PRODUCTS.length}
            </span>
          </button>
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-full ${on ? "bg-black/20" : "bg-white/8 text-slate-500"}`}
                >
                  {n}
                </span>
              </button>
          ...
          <div className="space-y-0.5">
            ...
            {currentCat.subcategories.map((sub) => {
              const n = PRODUCTS.filter(
                (p) => p.category === activeCategory && p.subcat === sub,
              ).length;
              if (!n) return null;
              return (
                <button
                  key={sub}
                  onClick={() =>
                    handleSubcat(sub === activeSubcat ? null : sub)
                  }
      ...
      <div className="bg-white/4 rounded-xl px-3 py-2.5 border border-white/8 text-center">
        ...
        <span className="text-slate-500 text-xs">
          {" "}
          producto{filteredCount !== 1 ? "s" : ""} encontrado
          {filteredCount !== 1 ? "s" : ""}
        </span>
      </div>
...
function Pill({ label, onRemove, color = "amber" }) {
  return (
    <span
      className={`flex items-center gap-1.5 border text-xs font-bold px-2.5 py-1 rounded-full ${PC[color]}`}
    >
      {label}
      <button
        onClick={onRemove}
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-0.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {cat && (
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r ${cat.color} text-white`}
            >
        ...
        {/* Nombre — 1 línea fija */}
        <h3
          className="font-bold text-white text-sm sm:text-base leading-tight group-hover:text-amber-100 transition-colors"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 1,
        ...
        <p
          className="text-slate-500 text-xs hidden sm:block leading-snug"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 1,
            WebkitBoxOrient: "vertical",
      ...
      <div className="flex flex-col items-end gap-2 shrink-0">
        <div className="text-right">
          <div className="text-amber-400 font-black text-base sm:text-lg leading-tight">
            S/.{product.price}
          </div>
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "rating":
        ...
        break;
      case "discount":
        list.sort((a, b) => discOf(b) - discOf(a));
        break;
      case "newest":
        ...
        <div className="fixed inset-0 z-[60] flex lg:hidden">
          ...
          <div className="pp-drawer relative bg-[#0d1623] w-[290px] h-full flex flex-col border-r border-white/8 shadow-2xl">
            <div className="flex items-center justify-between px-4 py-4 border-b border-white/8 shrink-0">
              <span className="pp-bb text-lg text-white tracking-widest">
                FILTROS
              </span>
              <button
                onClick={() => setDrawerOpen(false)}
                className="w-8 h-8 flex items-center justify-center rounded-xl text-slate-400 hover:text-white hover:bg-white/10 text-xl transition-all"
              >
                ✕
              </button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-600 mb-3 flex-wrap">
            ...
            {activeSubcat && (
              <>
                <span>›</span>
                <span className="text-slate-400">{activeSubcat}</span>
              </>
            )}
          </div>
          ...
          {totalActive > 0 && (
            <div className="flex flex-wrap gap-2 mt-4">
              {activeCategory !== "todos" && (
                <Pill
                  label={`${currentCat?.emoji} ${currentCat?.name}`}
                  onRemove={() => handleCategory("todos")}
                  color="amber"
                />
              )}
              {activeSubcat && (
              ...
              {onlyDiscount && (
                <Pill
                  label="+20% dto"
                  onRemove={() => setOnlyDiscount(false)}
                  color="red"
                />
              )}
              {onlyStock && (
              ...
              {currentCat.subcategories.map((sub) => {
                ...
                return (
                  ...
                  >
                    {sub}
                    <span
                      className={`text-[10px] ${activeSubcat === sub ? "text-black/60" : "text-slate-600"}`}
                    >
                      {n}
                    </span>
                  </button>
      ...
      {activeCategory !== "todos" && (
        <section className="border-t border-white/8 py-10 sm:py-12 bg-white/[.015]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <p className="pp-bb text-2xl sm:text-3xl text-white mb-5 tracking-wide">
              OTRAS CATEGORÍAS
            </p>
