  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <svg
          key={n}
          className={`${sz} ${n <= Math.round(rating) ? "text-amber-400" : "text-slate-700"}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
export function ProductDetail({ product, onClose }) {
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div
        className="pd-ov absolute inset-0 bg-black/80 backdrop-blur-sm"
        onClick={onClose}
      />
      <div className="pd-box relative bg-[#0d1623] w-full sm:max-w-2xl max-h-[96vh] sm:max-h-[88vh] rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col shadow-2xl border border-white/10 z-10">
        ...
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-white/8 shrink-0 bg-white/3">
          <div className="flex items-center gap-2 min-w-0 flex-wrap">
            <span
              className={`shrink-0 text-xs font-bold px-2.5 py-1 rounded-full bg-gradient-to-r ${cat?.color} text-white`}
            >
              {cat?.emoji} {cat?.name}
            </span>
            {product.subcat && (
            ...
            <div className="sm:w-44 xl:w-52 bg-gradient-to-b from-white/5 to-transparent flex flex-col items-center justify-center gap-3 p-6 border-b sm:border-b-0 sm:border-r border-white/8 shrink-0">
              <div className="relative">
                <span
                  ...
                  style={{ animation: "floatE 3s ease-in-out infinite" }}
                >
                  {product.emoji}
                </span>
                <span className="absolute -top-1 -right-2 bg-rose-600 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full">
                  -{pct}%
                </span>
              </div>
              <span className="text-[10px] font-black bg-amber-500/15 border border-amber-500/30 text-amber-400 px-3 py-1 rounded-full">
                {product.badge}
              </span>
              ...
              {tab === "info" && (
                <div className="grid grid-cols-1 gap-1.5">
                  {product.features.map((f) => (
                    <div
                      key={f.label}
                      className="flex gap-3 bg-white/4 rounded-xl px-3 py-2 border border-white/5"
                    >
                      <span className="text-amber-400 font-black text-xs shrink-0 w-24">
                        {f.label}
                      </span>
              ...
              {tab === "incluye" && (
                <ul className="space-y-2">
                  {product.includes.map((item, i) => (
                    ...
                    >
                      <span className="w-5 h-5 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 rounded-full flex items-center justify-center text-[10px] font-black shrink-0">
                        ✓
                      </span>
                      {item}
                    </li>
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    {product.forGrade.map((g) => (
                      <span
                        key={g}
                        className="bg-white/8 border border-white/12 text-slate-200 text-sm px-3 py-1.5 rounded-xl font-semibold"
                      >
                        {g}
                      </span>
                    ))}
                  ...
                  {product.colors?.length > 0 && (
                    <>
                      <p className="text-xs text-slate-500 font-semibold uppercase tracking-wide mt-3">
                        Versiones / colores
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {product.colors.map((c, i) => (
                          <span
                            key={i}
                            className="bg-white/5 border border-white/10 hover:border-amber-500/50 text-slate-300 text-xs px-3 py-1.5 rounded-xl cursor-pointer transition-all"
                          >
                            {c}
                          </span>
                        ))}
              ...
              <div className="flex items-center gap-3 flex-wrap pt-1">
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5">
                  <button
                    ...
                    className="w-7 h-7 bg-white/10 hover:bg-white/20 text-white rounded-full font-bold text-sm transition-all"
                  >
                    −
                  </button>
                  ...
                  <button
                    ...
                    className="w-7 h-7 bg-amber-500 hover:bg-amber-400 text-black rounded-full font-bold text-sm transition-all"
                  >
                    +
                  </button>
                </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-shrink-0 h-[130px] bg-gradient-to-br from-white/5 via-white/[.02] to-transparent flex items-center justify-center overflow-hidden">
        <span className="text-5xl sm:text-6xl group-hover:scale-110 transition-transform duration-200 select-none">
          {product.emoji}
        </span>
      ...
      <div className="p-3 flex flex-col flex-1 gap-1.5">
        ...
        <span
          className={`self-start text-[10px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r ${cat?.color ?? "from-slate-500 to-slate-600"} text-white leading-tight truncate max-w-full`}
        >
        ...
        <div className="mt-auto pt-1.5 flex items-center justify-between gap-2">
          <div className="min-w-0 leading-tight">
            <span className="text-amber-400 font-black text-sm sm:text-base block">
              S/.{product.price}
            </span>
            <span className="text-slate-600 text-[10px] line-through block">
              S/.{product.oldPrice}
            </span>
          </div>
          <button
...
export default function Home() {
  ...
  // Navegación interna — funciona desde cualquier sección del Home
  const goSection = (id) => {
    document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });
  };

  // Ir a productos con categoría
  const goCat = (catId) => navigate(`/productos?categoria=${catId}`);

  const featured = PRODUCTS.filter((p) => p.isFeatured);
  const shownFeatured =
    activeCatFilter === "todos"
      ? featured
      : featured.filter((p) => p.category === activeCatFilter);
  const newProducts = PRODUCTS.filter((p) => p.isNew).slice(0, 4);
  const catFilters = CATEGORIES.filter((c) =>
    featured.some((p) => p.category === c.id),
  );
  ...
  return (
    ...
    >
      <style>{`
        ...
        .card-pulse { animation: pulse-ring 2.5s ease-in-out infinite; }
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 flex flex-col lg:flex-row items-center gap-10 lg:gap-14">
          ...
          <div className="flex-1 text-center lg:text-left">
            ...
            <h1 className="hm-bb text-5xl sm:text-6xl lg:text-7xl leading-[0.88] mb-5">
              <span className="gtext">TODO LO QUE</span>
              <br />
            ...
            <p className="text-slate-400 text-base sm:text-lg mb-7 max-w-lg mx-auto lg:mx-0">
              Útiles, limpieza, arte y tecnología. Delivery en Ayacucho en menos
              de 24 h.
            </p>
          ...
          <div className="w-full max-w-xs sm:max-w-sm lg:max-w-[360px] flex flex-col gap-3">
            ...
            <div className="bg-gradient-to-br from-rose-700/95 to-red-900/95 rounded-3xl p-5 shadow-2xl border border-rose-500/30">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="bg-black/30 text-white/80 text-xs font-bold px-2.5 py-0.5 rounded-full">
                  TERMINA EN
                </span>
              </div>
              <div className="flex gap-2 mb-4">
                {[
                  ...
                  <div key={t.l} className="flex-1 text-center">
                    <div className="bg-black/40 rounded-xl py-2.5 hm-bb text-4xl text-white tabular-nums leading-none">
                      {pad(t.v)}
                    </div>
                    <div className="text-rose-300 text-[10px] mt-1 font-semibold tracking-widest">
                      {t.l}
                    </div>
                  </div>
                ))}
              </div>
              <p className="text-rose-200 text-xs text-center mb-1.5">
                Kit Regreso a Clases COMPLETO
              </p>
              <div className="flex items-center justify-center gap-3 mb-3">
                <span className="hm-bb text-4xl text-amber-300">S/.108</span>
                <span className="line-through text-rose-400 text-sm">
                  S/.180
                </span>
      ...
      <section className="py-10 sm:py-12 bg-gradient-to-r from-indigo-950/90 via-violet-950/90 to-indigo-950/90 border-y border-violet-700/25">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-7">
            <p className="hm-bb text-3xl sm:text-4xl text-white mb-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            {[
              ...
              },
              {
                min: "S/.200",
                d: "25% OFF",
                desc: "Para colegios e instituciones",
              ...
              <div
                key={d.min}
                className={`relative rounded-2xl p-5 sm:p-6 text-center border transition-all
                ${
                  d.hi
              ...
              >
                ...
                <div
                  className={`hm-bb text-4xl bg-gradient-to-r ${d.color} bg-clip-text text-transparent`}
                >
                  {d.min}
                </div>
      ...
      <section id="categorias" className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            <p className="hm-bb text-4xl sm:text-5xl text-white mb-1">
              NUESTRAS CATEGORÍAS
            </p>
            <p className="text-slate-400 text-sm">
              Útiles, limpieza, arte, tecnología, lonchera y más
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {CATEGORIES.map((cat) => (
              ...
              >
                <div className="text-3xl sm:text-4xl mb-2 block">
                  {cat.emoji}
                </div>
                <p className="font-bold text-xs sm:text-sm text-white group-hover:text-amber-400 transition-colors leading-tight">
                  {cat.name}
                </p>
      ...
      <section id="ofertas" className="py-12 sm:py-14 bg-white/[.015]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              Kits completos a precios especiales para esta temporada
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {PROMOTIONS.map((promo) => (
              ...
              >
                ...
                <div className="inline-block bg-black/25 text-white text-xs font-black px-3 py-1 rounded-full mb-3 self-start">
                  {promo.badge}
                </div>
                ...
                <div className="flex items-end justify-between gap-2 flex-wrap mt-auto">
                  <div>
                    <div className="flex items-baseline gap-2">
                      <span className="hm-bb text-3xl text-white">
                        S/.{promo.promoPrice}
                      </span>
                      <span className="line-through text-white/45 text-sm">
                  ...
                  <a
                    href={`https://wa.me/51999999999?text=${encodeURIComponent("Hola! Me interesa el " + promo.title)}`}
                    target="_blank"
                    rel="noreferrer"
      ...
      <section className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-7">
            <div>
              <p className="hm-bb text-4xl sm:text-5xl text-white">
                ⭐ DESTACADOS
              </p>
            ...
            <Link
              to="/productos"
              className="text-amber-400 hover:text-amber-300 font-bold text-sm transition-colors self-start sm:self-auto"
            >
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button
              onClick={() => setActiveCatFilter("todos")}
          ...
          {shownFeatured.length === 0 ? (
            <div className="text-center py-14 text-slate-500 text-sm">
              No hay destacados en esta categoría
            </div>
          ) : (
        ...
        <section className="py-10 sm:py-14 bg-white/[.015]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex items-center gap-3 mb-7">
              <span className="bg-sky-500 text-white text-xs font-black px-3 py-1 rounded-full">
                NUEVO
              </span>
              <p className="hm-bb text-3xl sm:text-4xl text-white">
                RECIÉN LLEGADOS
              </p>
            </div>
      ...
      <section id="packs" className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              Kits completos para cada nivel educativo
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {GRADE_PACKS.map((pack) => (
              ...
              >
                ...
                <h3 className="font-black text-white text-sm mb-3 leading-tight">
                  {pack.grade}
                </h3>
                ...
                <div className="mt-4 pt-3 border-t border-white/8 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-slate-600 text-[10px] line-through">
                      S/.{pack.originalPrice}
                    </div>
                  </div>
                  <a
                    href={`https://wa.me/51999999999?text=${encodeURIComponent("Hola! Quiero el kit para " + pack.grade)}`}
                    target="_blank"
                    rel="noreferrer"
      ...
      <section id="tendencias" className="py-12 sm:py-14 bg-white/[.015]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              Lo más pedido esta temporada escolar
            </p>
          </div>
      ...
      <section className="py-14 sm:py-16 bg-gradient-to-br from-emerald-950/80 to-teal-950/80 border-y border-emerald-700/25">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="hm-bb text-4xl sm:text-5xl text-white mb-3">
            PIDE POR WHATSAPP
          </p>
          <p className="text-emerald-300/80 text-base sm:text-lg mb-7 max-w-lg mx-auto">
            Asesoría personalizada y cotizaciones para colegios e instituciones.
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <a
              href="https://wa.me/51999999999?text=Hola!%20Necesito%20cotización%20para%20mi%20institución"
              target="_blank"
              rel="noreferrer"
      ...
      <section id="testimonios" className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              Familias y docentes de Ayacucho confían en nosotros
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {TESTIMONIALS.map((t) => (
              ...
              >
                ...
                <p
                  className="text-slate-300 text-sm leading-relaxed mt-3 flex-1"
                  style={{
                    display: "-webkit-box",
                    WebkitLineClamp: 4,
                    WebkitBoxOrient: "vertical",
      ...
      <section className="py-12 sm:py-14 bg-white/[.015] border-y border-white/5">
        <div className="max-w-xl mx-auto px-4 text-center">
          ...
          <p className="hm-bb text-3xl sm:text-4xl text-white mb-2">
            OFERTAS EXCLUSIVAS
          </p>
          <p className="text-slate-400 text-sm mb-5">
            Recibe primero las promos de temporada escolar
          </p>
          ...
          <p className="text-slate-700 text-xs mt-3">
            Sin spam. Solo ofertas de verdad.
          </p>
        </div>
      </section>
