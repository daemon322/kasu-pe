...
export function SearchProvider({ children, products }) {
  ...
  return (
    <SearchContext.Provider
      value={{ query, setQuery, results, isOpen, setIsOpen, clear }}
    >
      {children}
    </SearchContext.Provider>
  );
    ...
    <span>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark
            key={i}
            className="bg-amber-400 text-black px-0.5 rounded font-black not-italic"
          >
            {part}
          </mark>
        ) : (
...
export function SearchBox({ onClose, autoFocus = false }) {
  ...
  const handleSelect = (product) =>
    go(`/productos?q=${encodeURIComponent(product.name)}`);
  const handleViewAll = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      {/* Input */}
      <div
        className={`flex items-center gap-2 bg-slate-800 border rounded-xl px-3 py-2.5 transition-all duration-200
        ${focused ? "border-amber-500 shadow-md shadow-amber-500/20" : "border-slate-700 hover:border-slate-600"}`}
      >
        <svg
          className={`w-4 h-4 flex-shrink-0 transition-colors ${focused ? "text-amber-400" : "text-slate-500"}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        {query ? (
          ...
          <kbd className="hidden sm:block text-slate-600 text-[10px] border border-slate-700 px-1.5 py-0.5 rounded font-mono flex-shrink-0">
            Enter
          </kbd>
        )}
      </div>
      ...
      {isOpen && query && (
        <div
          className="absolute top-full mt-2 left-0 right-0 min-w-[300px] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl shadow-black/70 overflow-hidden z-[100]"
        ...
        >
          {results.length > 0 ? (
            <>
              {/* Productos */}
              <div className="p-2">
                <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest px-2 py-1.5">
                  Productos
                </p>
              ...
              {[...new Set(results.map((r) => r.category))].filter((cat) =>
                ...
                <div className="px-2 pb-2 border-t border-slate-800 pt-2">
                  <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest px-2 py-1">
                    Categoría
                  </p>
...
function CartDrawer({ open, onClose }) {
  ...
  const progress =
    toNext > 0 ? Math.min(100, (cartTotal / (cartTotal + toNext)) * 100) : 100;
  ...
  return (
    <div className="fixed inset-0 z-[80] flex justify-end">
      ...
      />
      <div
        className="relative bg-slate-900 w-full max-w-[360px] h-full flex flex-col shadow-2xl border-l border-slate-800"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all text-xl"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-3 bg-slate-800 rounded-2xl p-3 border border-slate-700/50"
              >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button
                    ...
                    className="w-7 h-7 bg-slate-700 hover:bg-red-500/30 text-white rounded-full text-sm font-black transition-all active:scale-90"
                  >
                    −
                  </button>
                  ...
                  <button
                    ...
                    className="w-7 h-7 bg-amber-500 hover:bg-amber-400 text-black rounded-full text-sm font-black transition-all active:scale-90"
                  >
                    +
                  </button>
                </div>
...
function MobileDrawer({ open, onClose, isHome, activeLink, onNavClick }) {
  ...
  return (
    ...
    >
      ...
      {/* Panel lateral */}
      <div
        className="relative bg-slate-900 w-[280px] h-full flex flex-col shadow-2xl border-r border-slate-800 overflow-y-auto"
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest px-3 mb-2">
            Navegación
          </p>
          ...
          <div className="pt-2 border-t border-slate-800 mt-2">
            <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest px-3 mb-2 mt-3">
              Tienda
            </p>
            <Link
              to="/productos"
        ...
        <div className="px-4 py-4 border-t border-slate-800 space-y-3 flex-shrink-0">
          ...
          <p className="text-center text-slate-600 text-xs">
            📍 Ayacucho, Perú · Envíos a todo el país
          </p>
        </div>
      </div>
    </div>
...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* ── NAV BAR ──────────────────────────────────────────── */}
      <nav
        className={`nb-font sticky top-0 z-50 transition-all duration-300
        ${
          scrolled
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-5 flex items-center h-14 sm:h-16 gap-2 sm:gap-3">
          ...
          >
            <span className="text-xl sm:text-2xl group-hover:rotate-12 transition-transform duration-300 inline-block">
              ✏️
            </span>
